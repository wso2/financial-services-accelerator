...
CONSENTMGR_CONFIG_FILE_PATH=${CONSENTMGR_PATH}/WEB-INF/classes
...
if [ -d "${CONSENTMGR_PATH}" ]; then
  ...
  # Backing up configurations.properties if exists
    if [ -f "${CONSENTMGR_CONFIG_FILE_PATH}/configurations.properties" ]; then
      mv -f "${CONSENTMGR_CONFIG_FILE_PATH}/configurations.properties" "${WEBAPPS_PATH}"
      echo -e "Backup of the configurations.properties file complete! \n"
      echo -e "================================================\n"
    fi
...
# Restoring configurations.properties
if [ -f "${WEBAPPS_PATH}/configurations.properties" ]; then
  mv -f ${WEBAPPS_PATH}/configurations.properties ${CONSENTMGR_CONFIG_FILE_PATH}
  echo -e "Restoring backup configurations.properties file complete!"
fi
