...
tablename  IN ('fs_notification','fs_notification_event','fs_notification_error');
...
IF (enableLog) THEN
RAISE NOTICE 'WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP STARTED .... !';
RAISE NOTICE '';
END IF;
...
THEN
    ...
    SELECT count(1) INTO rowcount  from pg_catalog.pg_tables WHERE schemaname = current_schema() AND tablename IN ('auditlog_fs_event_notification_cleanup');
    IF (rowcount = 0)
    THEN
        IF (enableLog AND logLevel IN ('TRACE')) THEN
        RAISE NOTICE 'CREATING AUDIT TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP .. !';
        END IF;
        CREATE TABLE auditlog_fs_event_notification_cleanup as SELECT * FROM fs_notification WHERE 1 = 2;
        ALTER TABLE auditlog_fs_event_notification_cleanup ADD COLUMN AUDIT_TIMESTAMP TIMESTAMP DEFAULT NOW();
    ELSE
        IF (enableLog AND logLevel IN ('TRACE')) THEN
        RAISE NOTICE 'USING AUDIT TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP ..!';
        END IF;
    END IF;
END IF;
...
IF (enableLog) THEN
    RAISE NOTICE '';
    RAISE NOTICE 'CALCULATING EVENT NOTIFICATIONS ON FS_NOTIFICATION .... !';

    IF (enableLog AND logLevel IN ('TRACE','DEBUG')) THEN
    SELECT COUNT(1) INTO rowcount FROM fs_notification;
    RAISE NOTICE 'TOTAL EVENT NOTIFICATIONS ON FS_NOTIFICATION TABLE BEFORE DELETE: %',rowcount;
    END IF;

    IF (enableLog AND logLevel IN ('TRACE')) THEN
        SELECT COUNT(1) INTO cleanupCount FROM fs_notification A WHERE (STRPOS( LOWER(','||eventStatuses||','), ','||LOWER(A.STATUS)||',') > 0) AND
            ((clientIds = '') IS NOT FALSE OR STRPOS(LOWER(','||clientIds||','), ','||LOWER(A.CLIENT_ID)||',') > 0) AND
            A.UPDATED_TIMESTAMP < to_timestamp(lastUpdatedTime)
            OR A.RESOURCE_ID IN (
                SELECT B.RESOURCE_ID FROM FS_NOTIFICATION B LEFT JOIN FS_CONSENT C ON B.RESOURCE_ID = C.CONSENT_ID WHERE C.CONSENT_ID IS NULL
                    AND purgeNonExistingResourceIds
            );
    RAISE NOTICE 'TOTAL EVENT NOTIFICATIONS SHOULD BE DELETED FROM FS_NOTIFICATION: %',cleanupCount;
    ...
    RAISE NOTICE 'TOTAL EVENT NOTIFICATIONS SHOULD BE RETAIN IN FS_NOTIFICATION: %',rowcount;
    END IF;
END IF;
...
LOOP
    ...
    SELECT count(1) INTO rowcount  from pg_catalog.pg_tables WHERE schemaname = current_schema() AND tablename IN ('chunk_fs_event_notifications');
    IF (rowcount = 1)
    THEN
        IF (enableLog AND logLevel IN ('TRACE')) THEN
        RAISE NOTICE '';
        RAISE NOTICE 'DROPPING EXISTING TABLE chunk_fs_event_notifications  !';
        END IF;
        DROP TABLE chunk_fs_event_notifications;
    END IF;

    CREATE TABLE chunk_fs_event_notifications (NOTIFICATION_ID VARCHAR);

    INSERT INTO chunk_fs_event_notifications (NOTIFICATION_ID) SELECT NOTIFICATION_ID FROM fs_notification A WHERE (
        STRPOS( LOWER(','||eventStatuses||','), ','||LOWER(A.STATUS)||',') > 0) AND
       ((clientIds = '') IS NOT FALSE OR STRPOS(LOWER(','||clientIds||','), ','||LOWER(A.CLIENT_ID)||',') > 0) AND
       A.UPDATED_TIMESTAMP < to_timestamp(lastUpdatedTime) OR
       A.RESOURCE_ID IN (
            SELECT B.RESOURCE_ID FROM FS_NOTIFICATION B LEFT JOIN FS_CONSENT C ON B.RESOURCE_ID = C.CONSENT_ID WHERE C.CONSENT_ID IS NULL AND
    ...
    CREATE INDEX idx_chunk_fs_event_notifications ON chunk_fs_event_notifications (NOTIFICATION_ID);

    IF (enableLog AND logLevel IN ('TRACE'))
    THEN
    RAISE NOTICE '';
    RAISE NOTICE 'PROCEEDING WITH NEW CHUNK TABLE chunk_fs_event_notifications  %',chunkCount;
    RAISE NOTICE '';
    END IF;

    IF (enableAudit)
    THEN
    INSERT INTO auditlog_fs_event_notification_cleanup SELECT FSN.*, NOW() FROM fs_notification FSN , chunk_fs_event_notifications CHK WHERE FSN.NOTIFICATION_ID=CHK.NOTIFICATION_ID;
   	COMMIT;
	END IF;

    LOOP
        SELECT count(1) INTO rowcount from pg_catalog.pg_tables WHERE schemaname = current_schema() AND tablename IN ('batch_fs_event_notification');
        IF (rowcount = 1)
        THEN
        DROP TABLE batch_fs_event_notification;
        END IF;

        CREATE TABLE batch_fs_event_notification (NOTIFICATION_ID VARCHAR);

        INSERT INTO batch_fs_event_notification (NOTIFICATION_ID) SELECT NOTIFICATION_ID FROM chunk_fs_event_notifications LIMIT batchSize;
        ...
        -- ------------------------------------------------------
        DELETE FROM fs_notification_event where NOTIFICATION_ID in (select NOTIFICATION_ID from batch_fs_event_notification);
        GET diagnostics deleteCount := ROW_COUNT;
		COMMIT;

        IF (enableLog AND logLevel IN ('DEBUG','TRACE')) THEN
        RAISE NOTICE 'BATCH DELETE FINISHED ON fs_notification_event WITH : %',deleteCount;
        END IF;

        -- ------------------------------------------------------
        -- BATCH DELETE FS_NOTIFICATION_ERROR
        -- ------------------------------------------------------
        DELETE FROM fs_notification_error where NOTIFICATION_ID in (select NOTIFICATION_ID from batch_fs_event_notification);
        GET diagnostics deleteCount := ROW_COUNT;
		COMMIT;

        IF (enableLog AND logLevel IN ('DEBUG','TRACE')) THEN
        RAISE NOTICE 'BATCH DELETE FINISHED ON fs_notification_error WITH : %',deleteCount;
        END IF;

        -- ------------------------------------------------------
        -- BATCH DELETE FS_NOTIFICATION
        -- ------------------------------------------------------
        DELETE FROM fs_notification where NOTIFICATION_ID in (select NOTIFICATION_ID from batch_fs_event_notification);
        GET diagnostics deleteCount := ROW_COUNT;
		COMMIT;

        IF (enableLog AND logLevel IN ('DEBUG','TRACE')) THEN
        RAISE NOTICE 'BATCH DELETE FINISHED ON fs_notification WITH : %',deleteCount;
        END IF;

        DELETE FROM chunk_fs_event_notifications WHERE NOTIFICATION_ID in (select NOTIFICATION_ID from batch_fs_event_notification);

        IF (enableLog AND logLevel IN ('TRACE')) THEN
        RAISE NOTICE 'DELETED BATCH ON  chunk_fs_event_notifications !';
        END IF;
...
IF (enableLog) THEN
RAISE NOTICE '';
RAISE NOTICE 'WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP COMPLETED .... !';
RAISE NOTICE '';
END IF;
