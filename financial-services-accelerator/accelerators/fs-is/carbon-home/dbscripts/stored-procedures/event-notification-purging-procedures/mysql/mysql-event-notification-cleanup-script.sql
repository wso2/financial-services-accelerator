DROP PROCEDURE IF EXISTS `WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP`;

DELIMITER $$

CREATE PROCEDURE `WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP`(
...
-- EX : `SET lastUpdatedTime = 1660737878;`
    ...
    IF (enableLog)
    THEN
        SELECT 'WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP STARTED ... !' AS 'INFO LOG';
    END IF;
...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    IF (enableAudit)
    THEN
        IF (NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP' and TABLE_SCHEMA in (SELECT DATABASE())))
        THEN
            IF (enableLog AND logLevel IN ('TRACE')) THEN
                SELECT 'CREATING AUDIT TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP .. !';
            END IF;
            CREATE TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP SELECT * FROM FS_NOTIFICATION WHERE 1 = 2;
            ALTER TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP ADD COLUMN `AUDIT_TIMESTAMP` TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
        ELSE
            IF (enableLog AND logLevel IN ('TRACE')) THEN
                SELECT 'USING AUDIT TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP ..!';
            END IF;
        END IF;
    END IF;
...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- CALCULATING NOTIFICATION EVENTS IN FS_NOTIFICATION TABLE
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    IF (enableLog)
    THEN
        SELECT 'CALCULATING EVENT NOTIFICATIONS IN FS_NOTIFICATION TABLE .... !' AS 'INFO LOG';

        IF (enableLog AND logLevel IN ('DEBUG','TRACE'))
        THEN
            SELECT  COUNT(1)  into rowcount FROM FS_NOTIFICATION;
            SELECT 'TOTAL EVENT NOTIFICATIONS ON FS_NOTIFICATION TABLE BEFORE DELETE' AS 'DEBUG LOG',rowcount;
        END IF;

        IF (enableLog AND logLevel IN ('TRACE'))
        THEN
            SELECT COUNT(1) into @cleanupCount FROM FS_NOTIFICATION A WHERE FIND_IN_SET(A.STATUS, eventStatuses) > 0 AND
                   (clientIds = '' OR FIND_IN_SET(A.CLIENT_ID, clientIds) > 0) AND A.UPDATED_TIMESTAMP < FROM_UNIXTIME(lastUpdatedTime)
                    OR A.RESOURCE_ID IN (
                            SELECT B.RESOURCE_ID FROM FS_NOTIFICATION B
                                LEFT JOIN FS_CONSENT C ON B.RESOURCE_ID = C.CONSENT_ID WHERE C.CONSENT_ID IS NULL AND purgeNonExistingResourceIds
                        );
            SELECT 'TOTAL EVENT NOTIFICATIONS SHOULD BE DELETED FROM FS_NOTIFICATION' AS 'TRACE LOG', @cleanupCount;
        ...
        THEN
            ...
            SELECT 'TOTAL EVENT NOTIFICATIONS SHOULD BE RETAIN IN FS_NOTIFICATION' AS 'TRACE LOG', rowcount;
        END IF;
    END IF;
...
-- ------------------------------------------------------
    ...
    EVENT_NOTIFICATION_CHUNK_LOOP: REPEAT
        ...
        DROP TEMPORARY TABLE IF EXISTS CHUNK_FS_EVENT_NOTIFICATION;

        CREATE TEMPORARY TABLE CHUNK_FS_EVENT_NOTIFICATION SELECT NOTIFICATION_ID FROM FS_NOTIFICATION A WHERE FIND_IN_SET(A.STATUS, eventStatuses) > 0 AND
           (clientIds = '' OR FIND_IN_SET(A.CLIENT_ID, clientIds) > 0) AND A.UPDATED_TIMESTAMP < FROM_UNIXTIME(lastUpdatedTime)
            OR A.RESOURCE_ID IN (
                SELECT B.RESOURCE_ID FROM FS_NOTIFICATION B LEFT JOIN FS_CONSENT C ON B.RESOURCE_ID = C.CONSENT_ID
                WHERE C.CONSENT_ID IS NULL AND purgeNonExistingResourceIds
            ) LIMIT chunkSize;

        SELECT COUNT(1) INTO @chunkCount FROM CHUNK_FS_EVENT_NOTIFICATION;
        ...
        CREATE INDEX IDX_CHK_FS_EVENT_NOTIFICATION ON CHUNK_FS_EVENT_NOTIFICATION(NOTIFICATION_ID);

        IF (enableLog AND logLevel IN ('TRACE'))
        THEN
            SELECT 'PROCESSING CHUNK FS_NOTIFICATION STARTED .... !' AS 'TRACE LOG',@chunkCount ;
        END IF;

        IF (enableAudit)
        THEN
            INSERT INTO AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP SELECT FSN.*, CURRENT_TIMESTAMP() FROM FS_NOTIFICATION AS FSN INNER JOIN CHUNK_FS_EVENT_NOTIFICATION AS CHK WHERE FSN.NOTIFICATION_ID = CHK.NOTIFICATION_ID;
        END IF;

        EVENT_NOTIFICATION_BATCH_LOOP: REPEAT

            DROP TEMPORARY TABLE IF EXISTS BATCH_FS_EVENT_NOTIFICATION;

            CREATE TEMPORARY TABLE BATCH_FS_EVENT_NOTIFICATION SELECT NOTIFICATION_ID FROM CHUNK_FS_EVENT_NOTIFICATION LIMIT batchSize;

            SELECT COUNT(1) INTO @batchCount FROM BATCH_FS_EVENT_NOTIFICATION;
            ...
            -- ------------------------------------------------------
            DELETE A
            FROM FS_NOTIFICATION_EVENT AS A
                     INNER JOIN BATCH_FS_EVENT_NOTIFICATION AS B
                                ON A.NOTIFICATION_ID = B.NOTIFICATION_ID;

            IF (enableLog AND logLevel IN ('TRACE'))
            THEN
                SELECT 'BATCH DELETE FINISHED FOR FS_NOTIFICATION_EVENT :' AS 'TRACE LOG',  row_count();
            END IF;
            ...
            -- ------------------------------------------------------
            DELETE A
            FROM FS_NOTIFICATION_ERROR AS A
                     INNER JOIN BATCH_FS_EVENT_NOTIFICATION AS B
                                ON A.NOTIFICATION_ID = B.NOTIFICATION_ID;

            IF (enableLog AND logLevel IN ('TRACE'))
            THEN
                SELECT 'BATCH DELETE FINISHED FOR FS_NOTIFICATION_ERROR :' AS 'TRACE LOG',  row_count();
            END IF;
            ...
            DELETE A
            FROM CHUNK_FS_EVENT_NOTIFICATION AS A
                     INNER JOIN BATCH_FS_EVENT_NOTIFICATION AS B
                                ON A.NOTIFICATION_ID = B.NOTIFICATION_ID;
    ...
    IF (enableLog )
    THEN
        SELECT 'EVENT NOTIFICATION DELETE ON FS_NOTIFICATION, FS_NOTIFICATION_ERROR, FS_NOTIFICATION_EVENT COMPLETED .... !' AS 'INFO LOG';
    END IF;

    IF (enableLog AND logLevel IN ('DEBUG','TRACE'))
    THEN
        SELECT 'TOTAL EVENT NOTIFICATIONS ON FS_NOTIFICATION TABLE AFTER DELETE' AS 'DEBUG LOG', COUNT(1) FROM FS_NOTIFICATION;
    END IF;
    ...
    THEN
        ...
        ANALYZE TABLE FS_NOTIFICATION;
        ANALYZE TABLE FS_NOTIFICATION_ERROR;
        ANALYZE TABLE FS_NOTIFICATION_EVENT;
...
-- ------------------------------------------------------
    IF (enableLog)
    THEN
        SELECT 'WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP() COMPLETED .... !' AS 'INFO LOG';
    END IF;
