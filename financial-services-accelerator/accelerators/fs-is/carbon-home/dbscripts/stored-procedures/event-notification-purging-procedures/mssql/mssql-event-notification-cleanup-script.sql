CREATE OR ALTER PROCEDURE WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP (
...
WHERE TABLE_NAME IN ('FS_NOTIFICATION','FS_NOTIFICATION_EVENT','FS_NOTIFICATION_ERROR')
...
IF (@enableLog = 1)
BEGIN
SELECT '[' + convert(varchar, getdate(), 121) + '] WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP STARTED ... !' AS 'INFO LOG';
END;
...
IF (@enableAudit = 1)
BEGIN
	IF (NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP'))
	BEGIN
			IF (@enableLog = 1 AND @logLevel IN ('TRACE')) BEGIN
			SELECT '[' + convert(varchar, getdate(), 121) + '] CREATING AUDIT TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP .. !';
			END
			Select * into dbo.AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP  from  dbo.FS_NOTIFICATION where 1 =2;
			ALTER TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP ADD AUDIT_TIMESTAMP datetime DEFAULT CURRENT_TIMESTAMP;
	END
	ELSE
	BEGIN
			IF (@enableLog = 1 AND @logLevel IN ('TRACE')) BEGIN
			SELECT '[' + convert(varchar, getdate(), 121) + '] USING AUDIT TABLE AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP ..!';
			END
	END
END
...
---- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
---- CALCULATING EVENT NOTIFICATIONS IN FS_NOTIFICATION TABLE
---- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

IF (@enableLog = 1)
BEGIN
		SELECT '[' + convert(varchar, getdate(), 121) + '] CALCULATING EVENT NOTIFICATIONS IN FS_NOTIFICATION TABLE .... !';

		IF (@enableLog = 1 AND @logLevel IN ('DEBUG','TRACE'))
		BEGIN
		SELECT @rowcount = COUNT(1) FROM FS_NOTIFICATION;
		SELECT '[' + convert(varchar, getdate(), 121) + '] TOTAL EVENT NOTIFICATIONS ON FS_NOTIFICATION TABLE BEFORE DELETE :'+CAST(@rowCount as varchar);
		END

		IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
		BEGIN
        SELECT @cleaupCount = COUNT(1) FROM FS_NOTIFICATION A WHERE (CHARINDEX( ',' + LOWER(A.STATUS) + ',', LOWER(',' + @eventStatuses + ',')) > 0) AND
            ...
            A.RESOURCE_ID in (SELECT B.RESOURCE_ID FROM FS_NOTIFICATION B LEFT JOIN FS_CONSENT C ON B.RESOURCE_ID = C.CONSENT_ID WHERE C.CONSENT_ID IS NULL AND 1 = @purgeNonExistingResourceIdsInternal);

		SELECT '[' + convert(varchar, getdate(), 121) + '] TOTAL EVENT NOTIFICATIONS SHOULD BE DELETED FROM FS_NOTIFICATION : ' + CAST(@cleaupCount as varchar);
		...
		SELECT '[' + convert(varchar, getdate(), 121) + '] TOTAL EVENT NOTIFICATIONS SHOULD BE RETAIN IN FS_NOTIFICATION : ' + CAST(@rowCount as varchar);
		END
END
...
WHILE (1=1)
BEGIN
		IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'CHUNK_FS_EVENT_NOTIFICATION'))
		BEGIN
		DROP TABLE CHUNK_FS_EVENT_NOTIFICATION;
		END

		CREATE TABLE CHUNK_FS_EVENT_NOTIFICATION (NOTIFICATION_ID VARCHAR (255),CONSTRAINT CHUNK_FS_EVENT_NOTIFICATION_PRI PRIMARY KEY (NOTIFICATION_ID));

        INSERT INTO CHUNK_FS_EVENT_NOTIFICATION (NOTIFICATION_ID) SELECT TOP (@chunkSize) NOTIFICATION_ID FROM FS_NOTIFICATION A WHERE
		...
		IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
		BEGIN
		SELECT '[' + convert(varchar, getdate(), 121) + '] CHUNK TABLE CHUNK_FS_EVENT_NOTIFICATION CREATED WITH : '+CAST(@chunkCount as varchar);
		END

		IF (@enableAudit=1)
		BEGIN
		INSERT INTO dbo.AUDITLOG_FS_EVENT_NOTIFICATION_CLEANUP SELECT FSN.*, CURRENT_TIMESTAMP FROM FS_NOTIFICATION FSN , CHUNK_FS_EVENT_NOTIFICATION CHK WHERE FSN.NOTIFICATION_ID = CHK.NOTIFICATION_ID;
		END

		WHILE (1=1)
		BEGIN
			IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'BATCH_FS_EVENT_NOTIFICATION'))
			BEGIN
			DROP TABLE BATCH_FS_EVENT_NOTIFICATION;
			END

			CREATE TABLE BATCH_FS_EVENT_NOTIFICATION (NOTIFICATION_ID VARCHAR (255),CONSTRAINT BATCH_FS_EVENT_NOTIFICATION_PRI PRIMARY KEY (NOTIFICATION_ID));

			INSERT INTO BATCH_FS_EVENT_NOTIFICATION (NOTIFICATION_ID) SELECT TOP (@batchSize) NOTIFICATION_ID FROM CHUNK_FS_EVENT_NOTIFICATION ;
			...
			BEGIN
				...
				DELETE FS_NOTIFICATION_EVENT where NOTIFICATION_ID in (select NOTIFICATION_ID from  BATCH_FS_EVENT_NOTIFICATION);
				SELECT  @deleteCount= @@rowcount;

				IF (@enableLog = 1)
				BEGIN
				SELECT '[' + convert(varchar, getdate(), 121) + '] BATCH DELETE FINISHED ON FS_NOTIFICATION_EVENT WITH : '+CAST(@deleteCount as varchar);
				END
				...
				DELETE FS_NOTIFICATION_ERROR where NOTIFICATION_ID in (select NOTIFICATION_ID from  BATCH_FS_EVENT_NOTIFICATION);
				SELECT  @deleteCount= @@rowcount;

				IF (@enableLog = 1)
				BEGIN
				SELECT '[' + convert(varchar, getdate(), 121) + '] BATCH DELETE FINISHED ON FS_NOTIFICATION_ERROR WITH : '+CAST(@deleteCount as varchar);
				END
				...
				DELETE FS_NOTIFICATION where NOTIFICATION_ID in (select NOTIFICATION_ID from  BATCH_FS_EVENT_NOTIFICATION);
				SELECT  @deleteCount= @@rowcount;

				IF (@enableLog = 1)
				BEGIN
				SELECT '[' + convert(varchar, getdate(), 121) + '] BATCH DELETE FINISHED ON FS_NOTIFICATION WITH : '+CAST(@deleteCount as varchar);
				END

				DELETE CHUNK_FS_EVENT_NOTIFICATION WHERE NOTIFICATION_ID in (select NOTIFICATION_ID from BATCH_FS_EVENT_NOTIFICATION);

				IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
				BEGIN
				SELECT '[' + convert(varchar, getdate(), 121) + '] DELETED BATCH ON CHUNK_FS_EVENT_NOTIFICATION !';
				END
...
BEGIN
SELECT @rowcount = COUNT(1) FROM FS_NOTIFICATION;
SELECT '[' + convert(varchar, getdate(), 121) + '] TOTAL EVENT NOTIFICATIONS ON FS_NOTIFICATION TABLE AFTER DELETE :'+CAST(@rowCount as varchar);
END
...
IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
BEGIN
SELECT '[' + convert(varchar, getdate(), 121) + '] WSO2_FS_EVENT_NOTIFICATION_CLEANUP_SP COMPLETED .... !' AS 'INFO LOG';
END
