...
class AuthorisationBuilder {
    ...
    private params = [
            (AcceleratorTestConstants.ENDPOINT_PARAMETER)     : configuration.getISServerUrl() + AcceleratorTestConstants.AUTH_URL,
            (AcceleratorTestConstants.RESPONSE_TYPE_PARAMETER): AcceleratorTestConstants.AUTH_RESPONSE_TYPE,
            (AcceleratorTestConstants.REDIRECT_URI_PARAMETER) : configuration.getAppInfoRedirectURL(),
            (AcceleratorTestConstants.STATE_PARAMETER)        : UUID.randomUUID().toString(),
            (AcceleratorTestConstants.NONCE_PARAMETER)        : UUID.randomUUID().toString(),
            (AcceleratorTestConstants.SCOPE_PARAMETER)        : getScopeString([AcceleratorTestConstants.ApiScope.ACCOUNTS]),
            (AcceleratorTestConstants.AUD_PARAMETER)          : configuration.getConsentAudienceValue()
    ]
    ...
    AuthorizationRequest getAuthorizationRequest(String clientId = getClientID().getValue(), String consentId,
                                                 List<AcceleratorTestConstants.ApiScope> scopes, boolean isRegulatory) {
        ...
        if(isRegulatory){
            request = new AuthorizationRequest.Builder(new ResponseType(), new ClientID(clientId))
                    .responseType(ResponseType.parse(AcceleratorTestConstants.AUTH_RESPONSE_TYPE))
                    ...
                    .state(getState())
                    .customParameter("nonce", AcceleratorTestConstants.NONCE_PARAMETER)
                    .build()
        }else{
            request = new AuthorizationRequest.Builder(new ResponseType(), new ClientID(clientId))
                    .responseType(ResponseType.parse(AcceleratorTestConstants.RESPONSE_TYPE_PARAMETER))
                    .endpointURI(getEndpoint())
                    .redirectionURI(getRedirectURI())
                    .scope(new Scope(scopeString))
                    .state(getState())
    ...
    AuthorizationRequest getAuthorizationRequestWithCustomValues(String clientId = getClientID().getValue(),
                                                                 String consentId, List<AcceleratorTestConstants.ApiScope> scopes,
        ...
        if (isRegulatory) {
            request = new AuthorizationRequest.Builder(new ResponseType(params.get(AcceleratorTestConstants.RESPONSE_TYPE_PARAMETER)),
                    new ClientID(clientId))
                    .responseType(new ResponseType(params.get(AcceleratorTestConstants.RESPONSE_TYPE_PARAMETER)))
                    .endpointURI(new URI(params.get(AcceleratorTestConstants.ENDPOINT_PARAMETER)))
                    .redirectionURI(new URI(params.get(AcceleratorTestConstants.REDIRECT_URI_PARAMETER)))
                    .requestObject(generator.getSignedAuthRequestObject(getScopeString(scopes), new ClientID(clientId),
                            new Issuer(clientId), consentId))
                    .scope(new Scope(params.get(AcceleratorTestConstants.SCOPE_PARAMETER)))
                    .state(new State(params.get(AcceleratorTestConstants.STATE_PARAMETER)))
                    .customParameter("nonce", params.get(AcceleratorTestConstants.NONCE_PARAMETER))
                    .build()
    ...
    AuthorizationRequest getAuthorizationRequestWithCustomResponseType(String clientId = getClientID().getValue(), String consentId,
                                                                       List<AcceleratorTestConstants.ApiScope> scopesForRequestObject,
                                                                       List<AcceleratorTestConstants.ApiScope> scopesForRequest,
                                                                       boolean isRegulatory) {

        JWTGenerator generator = new JWTGenerator()
        String scopeStringForRequestObj = "${AcceleratorTestConstants.ApiScope.OPEN_ID.scopeString} " +
                "${String.join(" ", scopesForRequestObject.collect({ it.scopeString }))}"
        String scopeStringForRequest = "${AcceleratorTestConstants.ApiScope.OPEN_ID.scopeString} " +
     ...
     */
    AuthorizationRequest getOAuthRequestWithoutRedirectUri(String clientId, List<AcceleratorTestConstants.ApiScope> scopes, String consentId) {

        JWTGenerator generator = new JWTGenerator()
        return new AuthorizationRequest.Builder(new ResponseType(params.get(AcceleratorTestConstants.RESPONSE_TYPE_PARAMETER)),
                new ClientID(clientId))
                .responseType(new ResponseType(params.get(AcceleratorTestConstants.RESPONSE_TYPE_PARAMETER)))
                .endpointURI(new URI(params.get(AcceleratorTestConstants.ENDPOINT_PARAMETER)))
                .requestObject(generator.getSignedAuthRequestObject(getScopeString(scopes), new ClientID(clientId),
     ...
     * @return authorization_response
     */
    AuthorizationRequest getOAuthRequestWithoutScope(String clientId, List<AcceleratorTestConstants.ApiScope> scopes, String consentId) {
     ...
     * @return authorization_response
     */
    AuthorizationRequest getOAuthRequestWithDefinedCert(String clientId, List<AcceleratorTestConstants.ApiScope> scopes,
    ...
    AuthorizationRequest getAuthorizationRequestWithCustomAud(String clientId = getClientID().getValue(), String consentId,
        ...
        if(isRegulatory){
            request = new AuthorizationRequest.Builder(new ResponseType(), new ClientID(clientId))
                    ...
                    .redirectionURI(getRedirectURI())
                    .requestObject(generator.getSignedAuthRequestObject(scopeString, new ClientID(clientId),
                            new Issuer(clientId), consentId, AcceleratorTestConstants.STATE_PARAMETER,
                            AcceleratorTestConstants.NONCE_PARAMETER, AcceleratorTestConstants.RESPONSE_TYPE_PARAMETER,
                            "localhost:8243/aud", AcceleratorTestConstants.REDIRECT_URI_PARAMETER))
                    .scope(new Scope(scopeString))
                    .state(getState())
                    .customParameter("nonce", AcceleratorTestConstants.NONCE_PARAMETER)
                    .build()
    ...
    private ResponseType getResponseType() {
        if (responseType == null) {
            responseType = new ResponseType(AcceleratorTestConstants.AUTH_RESPONSE_TYPE)
        }
        return responseType
    }
    ...
    public static String getScopeString(List<AcceleratorTestConstants.ApiScope> scopes) {
        return "${AcceleratorTestConstants.ApiScope.OPEN_ID.scopeString} " +
                "${String.join(" ", scopes.collect({ it.scopeString }))}"
    }

    public static String getScopeStringWithoutOpenIdScope(List<AcceleratorTestConstants.ApiScope> scopes) {
