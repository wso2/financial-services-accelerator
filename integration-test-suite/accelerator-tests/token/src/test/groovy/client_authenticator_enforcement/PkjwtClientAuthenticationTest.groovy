 ...
 */
class PkjwtClientAuthenticationTest extends FSAcceleratorTest {
    ...
    void "Validate token request with expired client assertion"() {
        ...
        clientId = configuration.getAppInfoClientID()
        Response tokenResponse = getApplicationAccessTokenResponseWithCustomExp(AcceleratorTestConstants.PKJWT_AUTH_METHOD,
                clientId, [scope], expireTime)

        Assert.assertEquals(tokenResponse.statusCode(), AcceleratorTestConstants.STATUS_CODE_400)
        Assert.assertEquals(OBTestUtil.parseResponseBody(tokenResponse, AcceleratorTestConstants.ERROR),
                "invalid_request")
        Assert.assertTrue(OBTestUtil.parseResponseBody(tokenResponse, AcceleratorTestConstants.ERROR_DESCRIPTION).
    ...
    void "Validate token request for pkjwt client without client_assertion"() {
        ...
        clientId = configuration.getAppInfoClientID()
        Response tokenResponse = getApplicationAccessTokenResponseWithoutAssertion(AcceleratorTestConstants.PKJWT_AUTH_METHOD,
                clientId, [scope])
    ...
    void "Validate token request for pkjwt client without client_id"() {
        ...
        Response tokenResponse = getApplicationAccessTokenResponseWithoutClientId(AcceleratorTestConstants.PKJWT_AUTH_METHOD,
                [scope])
...
//        Assert.assertEquals(OBTestUtil.parseResponseBody(tokenResponse, "token_type"), ConnectorTestConstants.BEARER)
    ...
    void "pkjwt client authentication with both x-wso2-mutual-auth-cert header and client_assertion"() {
        ...
        Assert.assertNotNull(OBTestUtil.parseResponseBody(tokenResponse, "expires_in"))
        Assert.assertNotNull(OBTestUtil.parseResponseBody(tokenResponse, "scope"))
        Assert.assertEquals(OBTestUtil.parseResponseBody(tokenResponse, "token_type"), AcceleratorTestConstants.BEARER)
    }
