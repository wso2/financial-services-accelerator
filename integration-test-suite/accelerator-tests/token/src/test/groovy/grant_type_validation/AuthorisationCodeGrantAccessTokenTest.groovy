 ...
 */
class AuthorisationCodeGrantAccessTokenTest extends FSAcceleratorTest {
	...
	void "Generate authorization code grant access token with pkjwt authentication"() {
		...
		authoriseConsent()
		Response tokenResponse = getUserAccessTokenResponse(AcceleratorTestConstants.PKJWT_AUTH_METHOD, clientId,
				code, consentScopes)
	...
	void "Validate additional claims not binding to the id_token of user access token"() {
		...
		Assert.assertNull(mapPayload.get(AcceleratorTestConstants.CNF))
		Assert.assertNull(mapPayload.get(AcceleratorTestConstants.CONSENT_ID))
	}
	...
	void "Introspection call for user access token"() {
		...
		Assert.assertNull(TestUtil.parseResponseBody(tokenResponse, AcceleratorTestConstants.GRANT_TYPE))
		Assert.assertNotNull(TestUtil.parseResponseBody(tokenResponse, AcceleratorTestConstants.CNF))
	}
	...
	void "OB-857_Generate authorization code grant access token for Regulatory Application"() {
		...
		Response tokenResponse = getUserAccessTokenResponse(AcceleratorTestConstants.TLS_AUTH_METHOD, clientId,
				code, consentScopes)
	...
	void "OB-516_Generate authorization code grant access token without authorization code"() {
		...
		Response tokenResponse = TokenRequestBuilder.getUserAccessTokenResponse(AcceleratorTestConstants.PKJWT_AUTH_METHOD,
	...
	void "OB-518_Generate authorization code grant access token with an already used authorization code"() {
		...
		Response tokenResponseNew = TokenRequestBuilder.getUserAccessTokenResponse(AcceleratorTestConstants.PKJWT_AUTH_METHOD,
				consentScopes.stream().map { it.scopeString }.toList(), clientId, code)

		Assert.assertEquals(tokenResponseNew.statusCode(), AcceleratorTestConstants.STATUS_CODE_400)
		Assert.assertEquals(TestUtil.parseResponseBody(tokenResponseNew, AcceleratorTestConstants.ERROR_DESCRIPTION),
				"Inactive authorization code received from token request")
		Assert.assertEquals(TestUtil.parseResponseBody(tokenResponseNew, AcceleratorTestConstants.ERROR),
				AcceleratorTestConstants.INVALID_GRANT)
	}
